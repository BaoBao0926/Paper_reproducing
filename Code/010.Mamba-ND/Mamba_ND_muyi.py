"""
toy version of Mamba-ND, official github respository: https://github.com/jacklishufan/Mamba-ND
"""
...
class Block(nn.Module):
    def __init__(self,
                 dim, mixer_cls, norm_cls=nn.LayerNorm, fused_add_norm=False, residual_in_fp32=False, reversse=False,
    ...
    def forward(self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params = None,
        ...
        """
        # 和vision mamba比起来，主要就是这个由于n_dim_pos造成的维度变化为block最大的不一样，也是原文如何进行scan变化的代码
        h = w = 0
...
class VisionMamba(BaseModule):
    ...
    A PyTorch implement of : `An Image is Worth 16x16 Words: Transformer for Image Recognition at Scale <https://arxiv.org/abs/2010.11929>`_、
            ...
            if k in state_dict:
                if (shape_1 := curr_state_dict[k].shape) != (shape_2 := state_dict[k].shape):
                    if 'patch_embed' in k:
                        ...
                        state_dict[k] = state_dict[k].repeat(1, self.patch_resolution[0],
                                                             1)  # / self.patch_resolution[0]
                        idxes = torch.arange(self.patch_resolution[0]).view(1, -1, 1).repeat(1, 1, old_len).view(1, -1,
                                                                                                                 1).float()
