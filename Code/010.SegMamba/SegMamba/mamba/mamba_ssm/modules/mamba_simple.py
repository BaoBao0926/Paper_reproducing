...
class Mamba(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)   # 把
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v3":   # v3 type forward，backward and inter-wise direction
                ...
                # xz ： [B, L, D] D为 2*d_inner=2*expand*d_model [64, 32, 16, 8]
                # 简单来看，nslices是多少，间隔为 总数/nslice
                # 比如[0,1,2,3,4...35] with nslice=5 -> [0, 7， 14， 31， 28， 1...]
                # 也就是说 intercal = total token / nslice => nslice = total token/inteval = H*W*D/H*W = D
                # 所以这个nslice的默认指应该不对,所以作为一个forward里面定义的参数是不对的，应该由__init__作为传参定义
            ...
            elif self.bimamba_type == "v2":     # v2 type forward+backward
                A_b = -torch.exp(self.A_b_log.float())
                out = mamba_inner_fn_no_out_proj(
                    xz,
                    self.conv1d.weight,
