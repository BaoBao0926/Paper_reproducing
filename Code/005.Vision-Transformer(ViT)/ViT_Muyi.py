...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads,
        ...
        *的作用是 *后面的所有参数都必须要使用关键字传参
        ...
        :param num_classes: 有多少个分类，默认为1000
        ...
        :param pool: 这是我添加的，如果，cls和mean的输出预测，感觉我写的不是很简洁，但是能用就行
        """
        ...
        self.pool = pool
        self.Dropout = nn.Dropout(dropout)
        # 其实我这里有点问题？根据源码显示的是，如果选择mean得到，那就不要cls，但是从paper的图像上来看，都有cls，mean也是对所有包括cls进行的
        # 所以这里有点奇怪，不过我还是按照 如果是mean就不用cls写的代码
        # 我怀疑，可能这个是simpleVIT，最简单的玩具版本ViT，只是用来给人感受学习的，所以没有写的复杂,正常版本的是 即使是mean也有cls
        # 如果是 都有cls，那么整个VIT的下面的代码就可以写到一起了
        if self.pool ==  "mean":  # 输出使用的是平均池化
            ...
            # 注意，这里应该是用dim，而不是patch_dim
            self.cls_token = nn.Parameter(torch.randn(dim))
            # 按照我的理解，2d的位置编码不可以使用在这里，所以使用一个上面得到的可训练参数作为1d的位置编码，要考虑到cls的存在
            patch_number = int(image_height/patch_height)*int(image_width/patch_width)+1
            print(f"patch number is {patch_number}")
            self.pos_embedding = nn.Parameter(torch.randn(1, patch_number, dim))
            self.transformer = Transformer(dim=dim, depth=depth, heads=heads, dim_head=dim_head,
                                           mlp_dim=mlp_dim, dropout=dropout)
            self.to_latent = nn.Identity()
            self.linear_head = nn.Linear(dim, num_classes)
    ...
    def forward(self, img):
        ...
        if self.pool == "mean":
            ...
            x = x.mean(dim=1)
            x = self.to_latent(x)       # 其实没有很理解这个Identity()的作用
            ...
            b, n, _ = x.shape
            cls_tokens = repeat(self.cls_token, 'd -> b n d', b=b, n=1)
            # 把cls拼接到图片patch的前面, 要在dim=1，而不是最后一维度
            # [batch_size, cls+patch number, patch dim]，所以应该拼接在dim=1的维度上
