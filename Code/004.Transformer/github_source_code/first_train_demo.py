...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, x, target):
        ...
        true_dist.scatter_(1, target.data.unsqueeze(1).to(torch.int64), self.confidence)
        true_dist[:, self.padding_idx] = 0
        # mask = torch.nonzero(target.data == self.padding_idx)
        mask = torch.nonzero(torch.as_tensor(target.data == self.padding_idx, dtype=torch.int64))
        # temp = torch.tensor(target.data == self.padding_idx)
        # mask = torch.nonzero(temp.clone().detach())
        if mask.dim() > 0:
